...
def compute_fault_and_axial_surfaces(params):
    ...
    # Coordonnées de la faille en surface
    Y_faille = params.get("Y_faille", 30000)  # en m
    Z_faille = params.get("Z_faille", 3400)  # en m

    # Définition de la première rampe
    aramp1 = np.tan(beta)
    ...
    # Point d'inflexion vers la 2ème rampe
    Y_r2 = params.get("Y_r2", 20000)
    Z_r2 = ramp1(Y_r2)

    # Définition de la 2ème rampe
    aramp2 = np.tan(teta)
    ...
    # Point d'inflexion vers la 3ème rampe
    Y_r3 = params.get("Y_r3", 10000)
    Z_r3 = ramp2(Y_r3)

    # Définition de la 3ème rampe
    aramp3 = np.tan(omega)
    ...
    # Étendue de la zone d'étude m UTM
    Ymin = params.get("Ymin", 0)
    Ymax = params.get("Ymax", 30000)

    # Nombre de strates à déformer
    n_strata = params.get("n_strata", 20)
    Zhaut = params.get("Zhaut", 1800)  ### INUTILE

    # Nombre de points par strate au départ
    Ymax2 = params.get("Ymax2", 14000)
    di = params.get("di", 2000)
    dj = int((Ymax2 - Ymin - 1) / di)

    # Nombre d'itérations de raccourcissement
    ite_s = 1
    ...
    # Points de sortie de charnière sur la rampe au nord
    ...
    # Droite où se trouveront l'ensemble des centres de rotation
    abis = coef
    ...
    def Asurf2(x):
        """
        Surface axiale interne nord
        """
        return coef * x + b_surf2

    def Asurf3(x):
        """
        Surface axiale interne sud
        """
    ...
    # Limitation de la taille des surfaces axiales pour représentation graphique
    ...
    for n in range(1, n_tot + 1):
        ...
        C_Y = Ypoint - (Rc * np.sin(teta))  #####################A VOIR##########################
        C_Z = Zpoint + (Rc * np.cos(teta))  #####################A VOIR##########################
        Zpoint2 = Zpoint + (Rc * np.cos(teta)) - (Rc * np.cos(beta))
        Ypoint2 = Ypoint - (Rc * np.sin(teta)) + (Rc * np.sin(beta))

        # Points localisés dans la zone 1
        for l in range(di + 1):
    ...
    return {
        ...
        "Y_r3": Y_r3,
        "Z_r3": Z_r3,
        # "Ymax_intersect": Ymax,  # Intersection avec Ymax
        # "Zmax_intersect": ramp1(Ymax),
        # "Ymax2_intersect": Ymax2,  # Intersection avec Ymax2
        "Zmax2_intersect": ramp2(Ymax2),
        "n_strata": n_strata,
        "Zhaut": Zhaut,
        "dj": dj,
        "ite_s": ite_s
...
if __name__ == "__main__":
    params = {
        ...
        "teta": 31,  # Deuxième segment plus plat
        "Y_r2": 26000,

        "omega": 30,  # Troisième segment presque horizontal
        "Y_r3": 13000,

        "Ymin": 6000,  # Début du calcul de la faille
        "Ymax": 29000,
    ...
    ax2.scatter(results["Ych1"], results["Zch1"], color='green', s=50, label='Charnière 1')
    ax2.scatter(results["Ych2"], results["Zch2"], color='blue', s=50, label='Charnière 2')
    ax2.scatter(results["Ych3"], results["Zch3"], color='orange', s=50, label='Charnière 3')
    ax2.scatter(results["Ych4"], results["Zch4"], color='purple', s=50, label='Charnière 4')
    ax2.scatter(results["Ycrois"], results["Zcrois"], color='cyan', s=50, label='Croisement ramp1-ramp2')
